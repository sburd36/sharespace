...
const insert = "insert into users(email, passHash, userName, firstName, lastName, photoURL) values(?,?,?,?,?,?)"
...
func (s *MySQLStore) PopTrie(trie *indexes.Trie) error {
	rows, err := s.db.Query(get)
	if err != nil {
		return fmt.Errorf("uncessfull retrival of users: %v", err)
	}
	for rows.Next() {
		user := &User{}
