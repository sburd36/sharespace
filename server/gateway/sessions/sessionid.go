...
func NewSessionID(signingKey string) (SessionID, error) {
	...
	if len(signingKey) == 0 {
		return InvalidSessionID, fmt.Errorf("signing key must not be empty 400: %v", http.StatusUnauthorized)
	}
	...
	// generating idLenghth radom bytes
	key := []byte(signingKey)
	randBytes := make([]byte, idLength)
	_, err := rand.Read(randBytes)
	if err != nil {
		return InvalidSessionID, fmt.Errorf("Server error, unable to encrypt 500: %v", err)
	}
	...
	randBytes = append(randBytes, signature...)
	// for _, s := range signature {
	// 	randBytes = append(randBytes, s)	
	// }

	newID := SessionID(base64.URLEncoding.EncodeToString(randBytes))
